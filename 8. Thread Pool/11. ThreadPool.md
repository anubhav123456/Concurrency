...
# ExecutorService – Essential Lifecycle & Scheduling Notes ⚙️⏰

## 1. ExecutorService Lifecycle Management

Lifecycle management is **critical** because thread pools are system resources.
If you don’t shut them down properly → **memory leaks, JVM not exiting, resource exhaustion**.
...
* If task throws exception → task is lost
* No retry mechanism
* If JVM crashes → task never executes
...
* Exception → future executions stop
* Long-running task + small period → overlap (if multiple threads exist)
* JVM crash → all schedules lost (no persistence)

---

### 🆚 Fixed Rate vs Fixed Delay (Interview Tip)
...
## 3. Creating Executors Using Executors Factory 🏭
...
## 4. Why Multiple Single-Thread Executors? (Advanced Concept 🔥)
...
> “I want request A to always go to thread X”
> → SingleThreadExecutor is perfect.
