...
👉 Common real-world example: **Producer–Consumer**, **Kafka consumers**, **shared queues**

---

## 🔑 Core Concepts

### 1️⃣ Monitor (Intrinsic Lock)
...
⚠️ Risk: Other threads may **wait forever**
...
✔ Must be called inside `synchronized`
✔ Must use **shared condition variable**
✔ Must re-check condition using **while**, not `if`
✔ Notification ≠ immediate execution (lock must be free)
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // One thread remains waiting → wake it explicitly
        sleep(2000);
...
⚠️ **Execution order may vary** due to thread scheduling.
...
2. Condition not met → `wait()`
3. Lock released
4. Another thread acquires lock → `notify / notifyAll`
...
✅ Prefer `notifyAll()` unless you're 100% sure
✅ Always guard conditions with `while`
✅ Improper notify can cause **deadlocks & resource leaks**
