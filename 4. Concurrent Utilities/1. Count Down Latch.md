...
## 1️⃣ What is `CountDownLatch` in Java?
...
## 3️⃣ Simple Example of `CountDownLatch`
...
## 4️⃣ Important Characteristics of `CountDownLatch`

✅ Multiple threads can call `await()`
✅ Multiple threads can call `countDown()`
❌ **Cannot be reused** once count reaches zero
✅ Thread-safe & part of `java.util.concurrent`

---

## 5️⃣ What is `Thread.join()`?
...
## 6️⃣ Difference Between `CountDownLatch` and `join()`

| Feature          | `CountDownLatch`                    | `join()`                 |
| ---------------- | ----------------------------------- | ------------------------ |
| Package          | `java.util.concurrent`              | `java.lang.Thread`       |
| Waiting for      | Multiple threads/tasks              | Single thread            |
| Reusability      | ❌ Not reusable                      | N/A                      |
| Multiple waiters | ✅ Yes                               | ❌ No                     |
| Decoupling       | ✅ Tasks don’t need thread reference | ❌ Requires thread object |
| Use case         | Parallel task coordination          | Simple thread waiting    |
| Timeout support  | ✅ `await(timeout)`                  | ✅ `join(timeout)`        |

---

## 7️⃣ Practical Comparison Example
...
❌ Tight coupling
❌ Must keep references of all threads
...
✅ Clean design
✅ Works with thread pools, async tasks, microservices-style logic
